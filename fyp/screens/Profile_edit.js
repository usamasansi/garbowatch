...
const Profile_edit = ({navigation, route}) => {
  ...
  const handleUpdateProfile = async () => {
    try {
      const response = await fetch('http://192.168.238.196:3000/api/profile', {
        method: 'POST',
        headers: {
    ...
    <View name="12">
      <View style={styles.garbowatch}>
        ...
        <LinearGradient
          ...
          <Text style={styles.garbowatch1}> Edit Profile</Text>
        </LinearGradient>
      ...
      <View style={{flexDirection: 'row', alignItems: 'center'}}>
        <Text style={{fontSize: 18, marginVertical: 10, color: 'grey'}}>
          {' '}
          I want to organize Cleaning Action
        </Text>
        <CheckBox
      ...
      <View style={{flexDirection: 'row', alignItems: 'center', marginTop: 10}}>
        <Text style={{flex: 1, color: 'gray', fontSize: 18}}>
          Location Format Degrees Example
        </Text>
      </View>
    </View>
  );
